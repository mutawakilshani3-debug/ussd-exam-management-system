...
export default function ExamSchedule() {
  ...
  return (
    <DashboardLayout title="Exam Schedule">
      {entries.map((entry) => (
        <div className="docket" key={entry.id}>
          <div>
            ...
            <div className="docket-meta">{entry.venue} · Invigilator: {entry.invigilator_name || 'Unassigned'}</div>
      ...
      {entries.length === 0 && <div className="empty-state">No exams assigned yet.</div>}
    </DashboardLayout>
  );
}
